...
async def start_duty_menu(callback: types.CallbackQuery):kb = InlineKeyboardMarkup(inline_keyboard=[(InlineKeyboardButton(text="🔮 Джинн Мустафа", callback_data="runquest_djinn_1")),(InlineKeyboardButton(text="🧠 Маг Афанасий", callback_data="runquest_mag_1")),(InlineKeyboardButton(text="🐍 Наг Гавриил", callback_data="runquest_nag_1")),(InlineKeyboardButton(text="🔥 Левиафан Дык", callback_data="runquest_leviafan_1"))])
await callback.message.edit_text("С кем из хранителей Системы вы разделите ночное дежурство?", reply_markup=kb)@dp.callback_query(lambda c: c.data.startswith("runquest_"))
async def handle_quests(callback: types.CallbackQuery):quest_key = callback.data.replace("runquest_", "")quest_data = MULTILEVEL_QUESTS.get(quest_key)
if quest_data:kb = InlineKeyboardMarkup(inline_keyboard=[(InlineKeyboardButton(text=t, callback_data=f"ans_{c}")) for t, c in quest_data("buttons"))
await callback.message.edit_text(quest_data("text"), reply_markup=kb)@dp.callback_query(lambda c: c.data.startswith("ans_"))
async def handle_answers(callback: types.CallbackQuery):user_id = callback.from_user.idaction = callback.data.replace("ans_", "")
if action == "wrong":kb = InlineKeyboardMarkup(inline_keyboard=[(InlineKeyboardButton(text="Попробовать снова", callback_data="start_duty")))
...
elif action.startswith("right_") and (action.endswith("2") or action.endswith("3")):next_step = action.replace("right", "")kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="Продолжить дежурство ➡️", callback_data=f"runquest{next_step}")]])
...
elif action == "right_final":p = db_get_player(user_id)new_prana = p("prana") + 300db_update_player(user_id, prana=new_prana)kb = InlineKeyboardMarkup(inline_keyboard=[(InlineKeyboardButton(text="В штаб Ашрама 🏰", callback_data="back_main")))
await callback.message.edit_text(f"🎉 ПОЛНАЯ ПОБЕДА НАД ПОДПЛАНАМИ!\n\nВы полностью очистили сектор ЛЭП, проявив глубочайшие ведические знания. Система BSS стабилизирована.\n\nНаграда: +300 Праны!\nБаланс: {new_prana} Праны.", reply_markup=kb)@dp.callback_query(lambda c: c.data == "back_main")
async def back_to_main(callback: types.CallbackQuery):p = db_get_player(callback.from_user.id)kb = InlineKeyboardMarkup(inline_keyboard=[(InlineKeyboardButton(text="🏰 Войти в Ашрам (База)", web_app=WebAppInfo(url=NGROK_URL))),(InlineKeyboardButton(text="⚔️ Начать дежурство (Квесты)", callback_data="start_duty"))])
